...
> **보강 (v3.1):** 2026-05-12 (§13 자가 점검 — 채움 메타포 재발견 + Sub-Phase B 진입 전 체크 항목 추가)
...
## 13. (2026-05-12 보강) 자가 점검 — 채움 메타포 재발견

> Sub-Phase A (5주 학습) Day 2에서 발생한 자가 발견. Phase 0 매핑 정의의 재확인, Sub-Phase B 진입 전 체크 항목 명문화, 5/8 무의식 결정 패턴 기록.

### 13.1 사건 요약

- **5/12 학습 Day 2** (Sub-Phase A): R3F + Three.js 학습 중 4종 mood 실험(ambient / directional / point / 조합). `pointLight`를 표면 가까이 배치 시 "안에서 빛나는" 효과 관찰.
- 이 위에서 "**빈 그릇 → 감정이 오브를 채움**" 메타포를 *자체적으로 떠올림*. 발표 발언 톤으로는 "큰 인사이트".
- 검토 결과: 2주 전(2026-04-28) 본인이 작성한 `specs/2026-04-28-living-orb-and-tone-design.md`의 5단계 + 4축 + opacity 로그 곡선이 **이미 같은 메타포**를 명시함.

### 13.2 기존 Phase 0 매핑 정의 — 재확인 (그대로 보존)

본 plan §1.6 "Out of Scope: Phase 0의 데이터 매핑 정의 변경"이 여전히 유효함. 5/12 학습이 그 결정의 정당성을 직관적으로 검증한 셈.

WebGL 전환 시 그대로 보존되는 매핑 (재확인):

| 축 | 정의 (Phase 0 spec) | 5/12 직관 검증 |
|---|---|---|
| `opacity` | `f₁(n) = 0.15 + 0.85 × log(n+1) / log(31)` (record 누적) | "1회 측정도 감지 가능한 변화" — 직관 일치 |
| `hue` | 주간 dominant 감정 가중 평균 (happy/calm/sad/angry) | 4 mood 실험에서 색 온도가 분위기 결정 — 직관 일치 |
| `saturation` | 주간 감정 강도 (negativeRatio, flatAffect) | "음영 = 살아있음의 신호" 발견 — 직관 일치 |
| `motion` | 측정 빈도 (최근 7일 일수 / 7) | 5/12에서는 미실험 |
| `edge softness` | (본 plan §3.6에서 추가) | 5/12에서는 미실험 |

→ 채움 메타포의 4가지 시각 변수 모두 Phase 0 spec에 정의되어 있음. 5/12 학습이 그 중 3개를 직관적으로 재발견함.

### 13.3 Sub-Phase B 진입 전 체크 항목 (5/12 보강)

§1.7 선결 조건에 다음을 추가:

- [ ] `specs/2026-04-28-living-orb-and-tone-design.md` 5단계 + 4축 표 (line 63-90) 재확인
- [ ] 본 plan §1.6 명시적 비목표 재확인 — 매핑 정의 변경 시도 없는지 점검
- [ ] 본 plan §12 Phase 0 결정 보존 목록 재확인
- 충돌 발견 시: 변경하지 말고 §10 잔여 항목에 보고 후 사용자 결정.

### 13.4 5/8 → 5/12 흐름 (무의식 결정 패턴 기록)

- **2026-05-04 (v3 작성)**: Tier 시스템 + 셰이더 성능 원칙 + 검증 3축. Phase 0 결정은 §12로 보존 명시.
- **2026-05-08 (무의식 시점)**: WebGL 전환을 "표현 매체 교체" 이상으로 확장 가능한지 무의식적으로 검토. Phase 0 spec 미확인 — 자가 점검 시 발견된 패턴.
- **2026-05-11 (학습 Day 1)**: R3F 환경 작동 확인 (회전 큐브).
- **2026-05-12 (학습 Day 2 + 자가 점검)**: 빛 모델 실험 중 채움 메타포 직관 재발견 → 기존 spec 재확인 → "재발견" 정체 인지 → 본 §13 보강.

**교훈**: 학습이 디자인을 만든 게 아니라, 학습이 **기존 디자인의 강도를 검증**한 사례. 본 plan 매핑 정의의 신뢰도를 추가로 뒷받침함.

### 13.5 향후 결정 시스템 (의무 체크)

ONMAUM 모든 신규 결정/방향 전환 시 다음을 의무화:

1. `docs/superpowers/specs/` 최근 3개 spec 재확인
2. `phases/*/00-phase-plan.md`의 §1.6 (Out of Scope)와 §12 (Phase 0 결정 보존) 재확인
3. 충돌 시 결정 변경하지 말고 사용자 결정 요청

본 워크플로는 §10 잔여 항목 처리와 합치. 명시적 비목표 위반은 자가 점검 시점에 잡힘.

---

*문서 끝. v3 시점 잔여: §10 #2 (영상 후보) Sub-Phase B 진입 전 사용자 수집 / §10 #4 (외부 디자이너 리뷰) Sub-Phase B 종료 시점 활용. v3.1 (2026-05-12 보강) 추가: §13 자가 점검 — 채움 메타포 재발견 + Sub-Phase B 진입 전 체크 항목 + 향후 결정 시스템 의무 체크.*
